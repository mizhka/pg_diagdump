...
# run pg_diagdump.sh
exec 5>&1
file_msg=$( sudo "$SD/pg_diagdump.sh" -p 5432 -C "$OUT_DIR" hang | tee >(cat - >&5) )
...
gcore_file=$(find "$OUT_DIR"/pg_results -name "diag_*_gcore*")
if [ ! -f "$gcore_file" ]; then
  echo "gcore file doesn't exist, file = $gcore_file"
  exit 1
fi
