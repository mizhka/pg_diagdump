...
pgbench -U postgres -d postgres -p 5433 -i || {
  echo "pgbench init failed"
  exit 1
}

# run pgbench to create work load
pgbench -U postgres -d postgres -p 5433 --time=600 --client=1 &> /dev/null &
pgbench_pid=$!
...
# run pg_diagdump.sh
exec 5>&1
file_msg=$( sudo "$PG_DIAGDUMP" -p 5433 -d "$OUT_DIR" state | tee >(cat - >&5) )
...
# get pid of process listening port 5433
pid_5433=$( sudo ss -tlpn | grep 5433 | grep -o -P '(?<=pid\=).*(?=\,fd)' | head -n 1 )
pid_5433_count=$(find "$OUT_DIR"/pg_results -name "*stacks_$pid_5433" | wc -l)
if [ "$pid_5433_count" == "0" ]; then
  echo "Error! Not stack file for port 5433."
  exit 1
fi
